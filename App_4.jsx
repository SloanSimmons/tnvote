...
function Landing({ onBrowseAll, onAddressResult }) {
  ...
  const handleLookup = async () => {
    ...
    try {
      ...
      if (data.error) {
        setError("Address not found. Try adding your city and zip code.");
        setLoading(false);
        return;
      }

      // Extract district numbers from OCD division IDs
      // e.g. "ocd-division/country:us/state:tn/sldu:29" → state senate district 29
      // e.g. "ocd-division/country:us/state:tn/county:shelby" → Shelby County
      const divisions = Object.keys(data.divisions || {});

      const sldu = divisions.find(d => /\/sldu:/.test(d)); // state senate
      const sldl = divisions.find(d => /\/sldl:/.test(d)); // state house
      const county = divisions.find(d => /\/county:/.test(d));

      const senateDistrict = sldu ? sldu.match(/\/sldu:(\d+)/)?.[1] : null;
      const countyName = county ? county.match(/\/county:(.+)/)?.[1] : null;
      const inShelby = countyName === "shelby";

      if (!inShelby) {
        setError("We currently only cover Shelby County, TN. Statewide coverage is coming soon.");
        setLoading(false);
        return;
      }

      // Filter candidates to only those relevant to this address
      const relevant = ALL_CANDIDATES.filter(c => {
        if (c.level === "county") return inShelby;
        if (c.level === "state" && c.office === "TN State Senate") {
          return senateDistrict && c.district === senateDistrict;
        }
        return false;
      });

      if (relevant.length === 0) {
        setError("No matching candidates found for this address. This may be outside our current coverage area.");
        setLoading(false);
        return;
      }
